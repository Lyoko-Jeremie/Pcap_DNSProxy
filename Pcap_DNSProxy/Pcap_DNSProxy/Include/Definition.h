...
#define CODEPAGE_UTF_32_BE                            12001U                      //UTF-32 Big Endian/BE
...
//10: UDP/IPv6
//11: UDP/IPv4
#define ALTERNATE_SERVER_NUM                          12U                         //Alternate switching of Main
//00: IPv6/UDP
//01: IPv4/UDP
//02: IPv6/TCP
//03: IPv4/TCP
#define TRANSPORT_LAYER_PARTNUM                       4U                          //Number of transport layer protocols
...
#define ADDRESS_STRING_MAXSIZE                        64U                               //Maximum size of address words, in bytes
...
#endif
#define DOMAIN_LABEL_MAXSIZE                          64U                               //Maximum size of domain label, in bytes
#define DOMAIN_POINTER_DEEPEST_LEVEL                  128U                              //Deepest level of domain
#define DOMAIN_RANDOM_MINSIZE                         6U                                //Minimum size of random domain
#define DOMAIN_WHOLE_MAXSIZE                          256U                              //Maximum size of whole domain, in bytes
...
#define NETWORK_LAYER_PARTNUM                         2U                                //Number of network layer protocols
#define NULL_TERMINATE_LENGTH                         1U                                //Length of C style string null
#define PACKET_ORIGINAL_MAXSIZE                       2048U                             //Maximum size of packet, in bytes
#define PATH_FILE_NAME_SIZE                           FILE_BUFFER_SIZE                  //Size of path file name reading buffer
#define PATH_FILE_NAME_MAXSIZE                        UINT16_MAX                        //Maximum size of file path length, in bytes.
#if defined(ENABLE_PCAP)
...
#endif
#define STRING_TO_DOMAIN_EXPAND_SIZE                  2U                          //Size of converting string to domain name format.
...
#define EDNS_RECORD_MAXSIZE                           (sizeof(uint8_t) + sizeof(dns_record_edns) + sizeof(edns_data_option) + sizeof(edns_cookies) + (sizeof(edns_data_option) + sizeof(edns_client_subnet)) * 2U + sizeof(in6_addr) + sizeof(in_addr))                    //Maximum of EDNS resource record size
#if defined(ENABLE_LIBSODIUM)
	#define DNSCRYPT_BUFFER_RESERVED_LEN                  (DNSCRYPT_MAGIC_QUERY_LEN + crypto_box_PUBLICKEYBYTES + crypto_box_HALF_NONCEBYTES - crypto_box_BOXZEROBYTES)
	#define DNSCRYPT_BUFFER_RESERVED_TCP_LEN              (sizeof(uint16_t) + DNSCRYPT_BUFFER_RESERVED_LEN)
	#define DNSCRYPT_DATABASE_ADDRESS_LOCATION            10U
	#define DNSCRYPT_DATABASE_PROVIDER_NAME_LOCATION      11U
	#define DNSCRYPT_DATABASE_PROVIDER_KEY_LOCATION       12U
	#define DNSCRYPT_PACKET_MINSIZE                       (DNSCRYPT_MAGIC_QUERY_LEN + crypto_box_NONCEBYTES + DNS_PACKET_MINSIZE)
	#define DNSCRYPT_HEADER_RESERVED_LEN                  (sizeof(ipv6_hdr) + sizeof(udp_hdr) + DNSCRYPT_BUFFER_RESERVED_LEN)
#endif
#define HTTP_1_RESPONSE_MINSIZE                       (strlen(" HTTP/") + HTTP_VERSION_MAXSIZE + HTTP_STATUS_CODE_SIZE)

//Network
#define DYNAMIC_MIN_PORT                              1024U                       //System port is from 1 to 1023.
#if defined(PLATFORM_WIN)
...
#if defined(ENABLE_LIBSODIUM)
	#define DNSCRYPT_DEFAULT_RECHECK_TIME                 1800U                                    //Default DNSCrypt keys recheck time, in seconds
	#define DNSCRYPT_DEFAULT_RELIABLE_SOCKET_TIMEOUT      DEFAULT_RELIABLE_ONCE_SOCKET_TIMEOUT     //Same as default timeout of reliable sockets
	#define DNSCRYPT_DEFAULT_UNRELIABLE_SOCKET_TIMEOUT    DEFAULT_UNRELIABLE_ONCE_SOCKET_TIMEOUT   //Same as default timeout of unreliable sockets
	#define DNSCRYPT_SHORTEST_RECHECK_TIME                10U                                      //The shortest DNSCrypt keys recheck time, in seconds
#endif
...
#define LOOP_MAX_TIMES                                8U                          //Maximum of loop times, in times
...
//Packet data
#if defined(PLATFROM_WIN)
	#define DEFAULT_ICMP_PADDING_DATA                     ("abcdefghijklmnopqrstuvwabcdefghi")
#endif
...
// 
#if defined(PLATFORM_WIN)
	#define COMMAND_FIREWALL_TEST                         (L"--firewall-test")
	...
	#define CONFIG_FILE_NAME_LIST_WCS                     {(L"Config.ini"), (L"Config.conf"), (L"Config.cfg")}
#if defined(ENABLE_LIBSODIUM)
	#define DNSCRYPT_KEY_PAIR_FILE_NAME                   (L"KeyPair.txt")
#endif
	#define ERROR_LOG_FILE_NAME_WCS                       (L"Error.log")
	#define FLUSH_DOMAIN_MAILSLOT_MESSAGE_ALL             (L"Flush Pcap_DNSProxy domain cache")          //Mailslot message to flush all domain cache
	#define FLUSH_DOMAIN_MAILSLOT_MESSAGE_SPECIFIC        (L"Flush Pcap_DNSProxy domain cache: ")        //Mailslot message to flush specific domain cache
	#define FLUSH_DOMAIN_MAILSLOT_NAME                    (L"\\\\.\\mailslot\\pcap_dnsproxy_mailslot")   //Mailslot name to flush domain cache
	#define MUTEX_UNIQUE_NAME                             (L"Global\\Pcap_DNSProxy process unique")      //Global mutex unique name
	#define SID_ADMINISTRATORS_GROUP                      (L"S-1-5-32-544")                              //Windows Administrators group SID header
	#define PROGRAM_SERVICE_NAME                          (L"PcapDNSProxyService")                       //Service name
#elif (defined(PLATFORM_FREEBSD) || defined(PLATFORM_LINUX) || defined(PLATFORM_MACOS))
	...
	#define CONFIG_FILE_NAME_LIST_WCS                     {(L"Config.conf"), (L"Config.ini"), (L"Config.cfg")}
	#define CONFIG_FILE_NAME_LIST_MBS                     {("Config.conf"), ("Config.ini"), ("Config.cfg")}
#if defined(ENABLE_LIBSODIUM)
	#define DNSCRYPT_KEY_PAIR_FILE_NAME                   ("KeyPair.txt")
#endif
	#define ERROR_LOG_FILE_NAME_WCS                       (L"Error.log")
	#define ERROR_LOG_FILE_NAME_MBS                       ("Error.log")
	#define FLUSH_DOMAIN_PIPE_MESSAGE_ALL                 ("Flush Pcap_DNSProxy domain cache")         //FIFO pipe message to flush all domain cache
	#define FLUSH_DOMAIN_PIPE_MESSAGE_SPECIFIC            ("Flush Pcap_DNSProxy domain cache: ")       //FIFO pipe message to flush specific domain cache
	#define FLUSH_DOMAIN_PIPE_PATH_NAME                   ("/tmp/pcap_dnsproxy_fifo")                  //FIFO pipe pathname to flush domain cache
#endif
