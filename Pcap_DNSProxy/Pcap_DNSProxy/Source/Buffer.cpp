...
bool PacketQueryToString(
	...
	const size_t DeepLevel, 
	std::vector<uint16_t> * PointerAlreadyUse, 
	std::string &StringName)
{
//Buffer and name size check
	if (PacketBuffer == nullptr || PacketLength <= sizeof(dns_hdr) || NameLength >= DOMAIN_WHOLE_MAXSIZE || NameOffset + NameLength > PacketLength)
		return false;
...
//Convert packet data to string.
	ssize_t LabelLength = 0;
	for (size_t Index = NameOffset;Index < NameOffset + NameLength;++Index)
	{
	//Name length check
	...
	//Label sign
		...
		//Message compression pointer
			...
			//Compression pointer length check
				if (Index + sizeof(uint8_t) >= NameOffset + NameLength)
					return false;

			//Locate pointer, it must point to a location before current.
				uint16_t PointerOffset = ((ntoh16(*reinterpret_cast<const uint16_t *>(PacketBuffer + Index))) & DNS_LABEL_POINTER_GET_LOCATION);
				if (PointerOffset < sizeof(dns_hdr) || PointerOffset >= NameOffset || PointerOffset + sizeof(uint8_t) >= PacketLength)
					return false;
			...
			//Level check and continue to copy data from pointer location.
				if (DeepLevel + 1U >= DOMAIN_POINTER_DEEPEST_LEVEL || GetDomainNameLength(PacketBuffer + PointerOffset, PacketLength - PointerOffset) == 0)
					return false;
				else 
					return PacketQueryToString(PacketBuffer, PacketLength, PointerOffset, GetDomainNameLength(PacketBuffer + PointerOffset, PacketLength - PointerOffset), DeepLevel + 1U, PointerAlreadyUse, StringName);
			}
		//Label length value
	...
	//Name data
			...
			StringName.push_back(PacketBuffer[Index]);
			LabelLength -= sizeof(uint8_t);
		}
	}
...
{
//Buffer and name size check
	if (PacketBuffer == nullptr || PacketSize == 0 || PacketSize == sizeof(uint16_t) || StringName.size() >= DOMAIN_WHOLE_MAXSIZE)
	{
		return 0;
	}
//Name is ROOT.
...
//Domain name compression are not allowed in resource record, except well-known type.
...
{
//Buffer and name size check
	if (PacketBuffer == nullptr || PacketLength <= sizeof(dns_hdr) || NameSize >= DOMAIN_WHOLE_MAXSIZE || NameOffset + NameSize > PacketLength)
		return false;
...
#define LABEL_PAIR_NAME_OFFSET           second
	...
	//End of name
		else if (PacketBuffer[Index] == 0)
		{
			...
			{
				CaseConvert(LabelPair.LABEL_PAIR_NAME_STRING, false);
	...
	//Label sign
		...
		//Message compression pointer
			...
			//Level check and continue to build name label stack from pointer location.
				if (DeepLevel + 1U >= DOMAIN_POINTER_DEEPEST_LEVEL || GetDomainNameLength(PacketBuffer + PointerOffset, PacketLength - PointerOffset) == 0)
					return false;
			...
			//Continue to build name label stack.
				return BuildPacketQueryStack(NameStack, PacketBuffer, PacketLength, PointerOffset, GetDomainNameLength(PacketBuffer + PointerOffset, PacketLength - PointerOffset), DeepLevel + 1U, PointerAlreadyUse);
			}
		//Label length value
			else if ((PacketBuffer[Index] & DNS_LABEL_GET_HIGH_2_BITS) == DNS_LABEL_NORMAL_8_BITS)
			{
...
//Name is ROOT or message compression pointer.
	else if (NameSize <= sizeof(uint16_t) || NameOffset + NameSize >= DNS_LABEL_POINTER_MAXSIZE)
		return true;
...
#define STACK_PAIR_NAME_OFFSET             second
	std::stack<std::pair<std::string, uint16_t>> NameStack;
	if (!BuildPacketQueryStack(NameStack, PacketBuffer, PacketLength, NameOffset, NameSize, 0, nullptr))
		return true;
...
//Packet message decompression
size_t PacketQueryDecompression(
	const uint8_t * const PacketBuffer, 
	const size_t PacketLength, 
	const uint16_t RecordType, 
	const size_t RecordOffset, 
	const size_t RecordSize, 
	uint8_t * const StorageOffset, 
	const size_t StorageSize)
{
	std::string RecordName;
	size_t CurrentOffset = 0, StorageLength = 0;

//Well-known types need to handle domain name compression.
//NS, CNAME, MB, MG, MR, PTR
	if (RecordType == DNS_TYPE_NS || 
		RecordType == DNS_TYPE_CNAME || 
		RecordType == DNS_TYPE_MB || 
		RecordType == DNS_TYPE_MG || 
		RecordType == DNS_TYPE_MR || 
		RecordType == DNS_TYPE_PTR)
	{
	//Name
		CurrentOffset = GetDomainNameLength(PacketBuffer + RecordOffset, RecordSize);
		if (CurrentOffset == 0 || CurrentOffset >= StorageSize)
		{
			return 0;
		}
		else if (CurrentOffset == sizeof(uint8_t))
		{
			memset(StorageOffset, 0, sizeof(uint8_t));
			return sizeof(uint8_t);
		}
		else if (!PacketQueryToString(PacketBuffer, PacketLength, RecordOffset, RecordSize, 0, nullptr, RecordName))
		{
			return 0;
		}
		else if (RecordName.empty() || (!RecordName.empty() && !CheckDomainAcceptable(RecordName)))
		{
			return 0;
		}

	//Copy back uncompression domain name.
		memset(StorageOffset, 0, StorageSize);
		StorageLength = StringToPacketQuery(RecordName, StorageOffset, StorageSize);
		if (StorageLength <= sizeof(uint16_t))
			return 0;
		else 
			return StorageLength;
	}
//SOA, MINFO
	else if (RecordType == DNS_TYPE_SOA || 
		RecordType == DNS_TYPE_MINFO)
	{
	//Primary Name or RMAILBX
		CurrentOffset = GetDomainNameLength(PacketBuffer + RecordOffset + CurrentOffset, RecordSize - CurrentOffset);
		if (CurrentOffset == 0 || CurrentOffset >= StorageSize)
		{
			return 0;
		}
		...
		{
			memset(StorageOffset + StorageLength, 0, sizeof(uint8_t));
			StorageLength += sizeof(uint8_t);
		}
		else if (!PacketQueryToString(PacketBuffer, PacketLength, RecordOffset, CurrentOffset, 0, nullptr, RecordName))
		{
			return 0;
		}
		else if (RecordName.empty() || (!RecordName.empty() && !CheckDomainAcceptable(RecordName)))
		...
		//Copy back uncompression domain name.
			memset(StorageOffset + StorageLength, 0, StorageSize - StorageLength);
			StorageLength += StringToPacketQuery(RecordName, StorageOffset + StorageLength, StorageSize - StorageLength);
			if (StorageLength <= sizeof(uint16_t))
				return 0;
		}

	//Mailbox Name or EMAILBX
		auto MailboxNameLength = GetDomainNameLength(PacketBuffer + RecordOffset + CurrentOffset, RecordSize - CurrentOffset);
		RecordName.clear();
		if (MailboxNameLength == 0 || MailboxNameLength >= StorageSize)
		{
			return 0;
		}
		else if (MailboxNameLength == sizeof(uint8_t))
		{
			CurrentOffset += MailboxNameLength;
			memset(StorageOffset + StorageLength, 0, sizeof(uint8_t));
			StorageLength += sizeof(uint8_t);
		}
		else if (!PacketQueryToString(PacketBuffer, PacketLength, RecordOffset + CurrentOffset, MailboxNameLength, 0, nullptr, RecordName))
		{
			return 0;
		}
		else if (RecordName.empty() || (!RecordName.empty() && !CheckDomainAcceptable(RecordName)))
		...
		//Copy back uncompression domain name.
			memset(StorageOffset + StorageLength, 0, StorageSize - StorageLength);
			MailboxNameLength = StringToPacketQuery(RecordName, StorageOffset + StorageLength, StorageSize - StorageLength);
			if (MailboxNameLength <= sizeof(uint16_t))
				return 0;
			else 
				StorageLength += MailboxNameLength;
		}

	//Other resource in record
		if (RecordType == DNS_TYPE_MINFO)
		{
			return StorageLength;
		}
		else if (CurrentOffset + sizeof(dns_record_soa) > RecordSize || StorageLength + sizeof(dns_record_soa) > StorageSize)
		{
			return 0;
		}
		else {
			memcpy_s(StorageOffset + StorageLength, StorageSize - StorageLength, PacketBuffer + RecordOffset + CurrentOffset, sizeof(dns_record_soa));
			return StorageLength;
		}
	}
//MX
	else if (RecordType == DNS_TYPE_MX)
	{
...
//Other records without compression.
	else if (RecordSize <= StorageSize)
	{
		memcpy_s(StorageOffset, StorageSize, PacketBuffer + RecordOffset, RecordSize);
		return RecordSize;
	}
