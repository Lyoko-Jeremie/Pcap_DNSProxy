...
//Open all devices
		if (pcap_findalldevs(&pThedevs, ErrBuffer) == RETURN_ERROR)
		{
			...
			PrintError(LibPcap_Error, wErrBuffer, NULL, NULL);
...
//Permissions check and check available network devices
		if (pThedevs == nullptr)
		{
			...
			if (!Non_Loopback)
			{
				if (Index == 89)
				{
					PrintError(LibPcap_Error, L"Not any available network devices", NULL, NULL);
...
//Open device(Linux/Mac)
	if ((pAdHandle = pcap_open_live(pDrive->name, PACKET_MAXSIZE, FALSE, TIME_OUT/4, Buffer)) == nullptr)
	{
		...
		PrintError(LibPcap_Error, ErrBuffer, NULL, NULL);
...
//Compile the string into a filter program(Linux/Mac)
	bpf_program FCode = {0};
...
//ICMP header options check(Linux/Mac)
...
//Send to localhost
	if (Index >= THREAD_MAXNUM*THREAD_PARTNUM/2) //TCP area
	{
		...
		send((int)SystemPort.Socket, TCPBuffer, Length + sizeof(uint16_t), NULL);
		delete[] TCPBuffer;
	}
	else { //UDP
		sendto((int)SystemPort.Socket, Buffer, Length, NULL, (sockaddr *)&(SystemPort.SockAddr), SystemPort.AddrLen);
	}
