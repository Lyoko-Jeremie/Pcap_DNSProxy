...
//Check socket
	...
	{
		PrintError(RETURN_ERROR, L"ICMP Echo(Ping) request error", errno, 0);
...
//Validate local IPv6 address
	...
	{
		PrintError(RETURN_ERROR, L"Get local IPv6 address error", 0, 0);
...
//Check socket
	if (Request == RETURN_ERROR)
	{
		PrintError(RETURN_ERROR, L"ICMPv6 Echo(Ping) request error", errno, 0);
...
//Socket initialization
	...
	{
		PrintError(RETURN_ERROR, L"TCP request initialization failed", errno, 0);
...
//Socket initialization
	...
	{
		PrintError(RETURN_ERROR, L"UDP request initialization failed", errno, 0);
...
//Send request
	...
	{
		PrintError(RETURN_ERROR, L"UDP request error", errno, 0);
