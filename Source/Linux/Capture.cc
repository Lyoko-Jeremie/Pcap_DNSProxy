...
//Initialization
	char *ErrBuf = nullptr;
	wchar_t *wErrBuf = nullptr;
	try {
		ErrBuf = new char[PCAP_ERRBUF_SIZE]();
		wErrBuf = new wchar_t[PCAP_ERRBUF_SIZE]();
	}
	catch (std::bad_alloc)
	{
	...
	memset(wErrBuf, 0, sizeof(wchar_t)*PCAP_ERRBUF_SIZE);

//Open all devices
	if (pcap_findalldevs(&pThedevs, ErrBuf) == RETURN_ERROR)
	{
		mbstowcs(wErrBuf, ErrBuf, strlen(ErrBuf));
		PrintError(LibPcap_Error, wErrBuf, 0, 0);
...
//Initialization
	...
	char *Addr = nullptr, *LocalAddr = nullptr, *Buffer = nullptr;
	try {
...
//Check device type
	...
	{
/* There are lots of preloaded no-Ethernet devices in system(Linux)
...
//Set capture filter
	...
	//When Local DNS server is not available, the filter is (src host (IPv4Target or IPv6Target)) or (pppoes and src host (IPv4Target or IPv6Target))"
		...
		if (Local)
			inet_ntop(AF_INET, (char *)&Parameter.DNSTarget.Local_IPv4Target, LocalAddr, PACKET_MAXSIZE/8);
		...
		if (Local)
			inet_ntop(AF_INET6, (char *)&Parameter.DNSTarget.Local_IPv6Target, LocalAddr, PACKET_MAXSIZE/8);
		BothAddr.append(Addr);
		if (Local)
		{
			BothAddr.append(" or ");
...
//Compile the string into a filter program
	...
	{
		mbstowcs(ErrBuffer, pcap_geterr(pAdHandle), strlen(pcap_geterr(pAdHandle)));
		PrintError(LibPcap_Error, ErrBuffer, 0, 0);
	...
	while(true)
	{
		...
		switch (Result)
		{
			case RETURN_ERROR: //An error occurred
			{
				...
				continue;
/* The device is offline, wait for retrying(Linux)
